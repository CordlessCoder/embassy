...
//! # Pins
//!
//! `PA16` and `PA17`, which on this board are two of the three adjacent header pins with no jumper, LED or
//! sensor on them (J2.24 and J2.25). Rig C's marker leads sit on them, and an analyser channel only
...
/// The pin this example drives, so that a wait can be given an edge of either direction.
const DRIVEN_BIT: usize = 16;

/// The pin that is only ever waited on, to put a second node in the list.
const OTHER_BIT: usize = 17;
...
fn inject(bit: usize) {
    pac::GPIOA.cpu_int().iset().write(|w| w.set_dio(bit, true));
...
fn drive(bit: usize, high: bool) -> bool {
    if high {
        pac::GPIOA.doutset31_0().write(|w| w.set_dio(bit, true));
    } else {
        pac::GPIOA.doutclr31_0().write(|w| w.set_dio(bit, true));
    }
    ...
    pac::GPIOA.din31_0().read().dio(bit) == high
}
...
async fn main(_spawner: Spawner) -> ! {
    ...
    let mut other = Input::new_async(p.PA17, Irqs, Pull::Up);
