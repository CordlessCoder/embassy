//! `i2c_faults` for the async controller on the L1306, which is a different error path and a different
//! family: the L-series I2C and SYSCTL blocks are not the G-series ones the same test covers there.
//!
//! Same wiring and same battery as the blocking copy: `PA1`→`PB8` SCL, `PA0`→`PB9` SDA, shared ground,
...
async fn main(_spawner: Spawner) -> ! {
    ...
    let mut i2c = unwrap!(I2c::new_async(p.I2C0, p.PA1, p.PA0, Irqs, config));
