...
//!
//! Five `ok` lines and nothing else. Every failure prints what it expected and stops that case — a hang
...
async fn across_ports(driven: &mut Flex<'static>, across: &mut Input<'static>) {
...
async fn main(_spawner: Spawner) -> ! {
    ...
    let mut driven = Flex::new(p.PB17);
    driven.set_as_input_output();
    driven.set_pull(Pull::Up);
    driven.set_high();

    let mut other = Input::new(p.PB0, Pull::Up);
    let mut across = Input::new(p.PA17, Pull::Up);
