...
//! `MAX_WAKE_NS` longer per sleep than one it rejects into `WFI`. On this part that is 15.2 µs, half a
//! tick — invisible in one sleep, about 500 ticks over a batch.
//!
//! Nothing but the sleep length changes between cases, so the step in the per-sleep cost is the gate.
//!
//! | Pin            | Signal                                                                 |
//! |----------------|------------------------------------------------------------------------|
//! | `PB3` + `PB13` | high for the batches the gate should allow, low for the ones it rejects |
//! | `PA0`          | on-board LED, toggled once per pass as a liveness check                 |
//!
//! The pin is what a current probe needs: supply current should drop to the STANDBY figure exactly
//! while it is high, and sit at RUN current while it is low. Both pins carry it because which one a
//! board breaks out varies — probe whichever is available. They pulse ten times at startup, so a flat
//! line means the pin is not reaching the probe rather than the run being broken.
//!
...
async fn main(_spawner: Spawner) -> ! {
    ...
    let mut allowed_pin = Output::new(p.PB3, Level::Low);
    let mut allowed_pin_alt = Output::new(p.PB13, Level::Low);
    let mut led = Output::new(p.PA0, Level::High);
    led.set_inversion(true);

    // Proves the pins reach the probe before a current trace is aligned against them.
    for _ in 0..10 {
        allowed_pin.set_high();
        allowed_pin_alt.set_high();
        Timer::after_millis(50).await;
        allowed_pin.set_low();
        allowed_pin_alt.set_low();
        Timer::after_millis(50).await;
    }
    ...
    loop {
        ...
        for &ticks in CASES {
            ...
            allowed_pin_alt.set_level(allowed.into());
