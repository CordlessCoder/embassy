...
//!
//! Other side of the MSPM0 `i2c_target` example, unmodified — it answers a read with `[8, 8]`, a
//! write-read with `[9, 9]` padded with `0xFE`, and has the general call enabled. Those fixed answers
//! are what makes it testable from here: an `embassy-stm32` controller is an independent
//! implementation, so agreement is evidence about our target driver rather than about one HAL agreeing
//! with itself.
//!
...
//!
//! The first four are asserted. The read of 4 is logged rather than checked: what a target sends after
//! it runs out of bytes is not defined by the driver, and the interesting part is that it recovers,
//! which shows up as the next pass passing.
