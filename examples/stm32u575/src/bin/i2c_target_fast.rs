//! `i2c_target` at 400 kHz, for testing an MSPM0 controller in fast mode.
//!
//! A binary of its own rather than a constant to edit: the slave's `TIMINGR` carries the data setup and
//! hold times for the bus speed, so a target left configured for 100 kHz is not a fair partner for a
//! 400 kHz controller — and C1, C2 and C4 all need the 100 kHz one to stay as it is.
//!
//! **Fast mode needs stronger pull-ups than standard mode.** Measured on this rig, 4.7 kΩ gives a rise
//! time of about 850 ns, which fits standard mode's 1000 ns limit and misses fast mode's 300 ns by three
//! times over. That works out at 82 pF of bus capacitance, so fast mode wants **1.5 kΩ or lower**.
//!
//! Other side of the MSPM0 `i2c_fastmode` example. The answers are deliberately identical to the ones
...
//! 1.5 kΩ to 3V3 on each of SCL and SDA:
//!
...
/// 400 kHz, matching `BusSpeed::FastMode` on the MSPM0 side.
const FREQUENCY: Hertz = Hertz::khz(400);
