//! H321x deep sleep: STOP0/2 + STANDBY0/1 (no STOP1).
//!
//! Covers mspm0h321x. It lacks the STOP1 (4 MHz SYSOSC) sub-mode, and its STOP0 clears `DISABLESTOP` only.
//!
//! The entry sequence from the TRM is:
//! `PMODECFG.DSLEEP` selects STOP vs STANDBY,
//! `SYSOSCCFG.DISABLESTOP` selects STOP0 vs STOP2 (no 4 MHz STOP1, and STOP0 clears DISABLESTOP only),
...
pub unsafe fn enter_sleep(_cs: CriticalSection, mode: SleepMode) {
    ...
    match mode {
        SleepMode::Stop0 => sysctl.sysosccfg().modify(|w| w.set_disablestop(false)),
